//
// This is the full content for api/payment-frame.js (v13 - Server-Side Allowance)
//
module.exports = async function handler(req, res) {
  console.log("[v13] /api/payment-frame called - Method:", req.method)
  ...
  try {
    ...
    if (!GAME_URL || !PUBLIC_URL) {
      console.error("[v13] ERROR: Missing GAME_URL or PUBLIC_URL env vars")
      return res.status(500).send("Server configuration error: Missing required environment variables.")
    }

    console.log("[v13] Payment frame loaded")
...
<body>
  ...
  <script type="module">
    console.log('[v13] Payment frame script starting')
    ...
    const usdcAbi = [
      ...
      // allowance is no longer needed here
    ]
    ...
    payButton.addEventListener('click', async () => {
      console.log('[v13] Button clicked!')
      ...
      try {
        ...
        console.log('[v13] Importing Farcaster SDK')
        const { default: sdk } = await import('https://esm.sh/@farcaster/miniapp-sdk')
        
        console.log('[v13] SDK imported, calling ready()')
        await sdk.actions.ready()
        
        console.log('[v13] Getting Ethereum provider from wallet')
        ...
        console.log('[v13] Provider obtained, requesting accounts')
        statusDiv.textContent = 'Connecting wallet...'
        
        const accounts = await provider.request({ method: 'eth_requestAccounts' })
        
        const rawUserAddress = accounts[0]
        console.log('[v13] User address (raw):', rawUserAddress)
        const userAddress = ethers.utils.getAddress(rawUserAddress)
        console.log('[v13] User address (checksummed):', userAddress)
        ...
        console.log(\`[v13] Fetching data from /api/get-price?userAddress=\${userAddress}\`)
        ...
        currentAllowance = ethers.BigNumber.from(data.allowance); // Get allowance from server
        
        console.log(\`[v13] Current price: \${price.toString()} (\${priceInUsdc} USDC)\`)
        console.log(\`[v13] Current epochId: \${epochId}\`)
        console.log(\`[v13] Current allowance: \${currentAllowance.toString()}\`)

        if (price.isZero()) {
          console.log('[v13] Price is zero. Switching to free claim.')
        ...
        if (price.gt(0)) { // Only run if price is greater than 0
          ...
          console.log('[v13] Price > 0. Checking allowance...')
          
          // We use the currentAllowance we fetched from the server
          if (currentAllowance.lt(price)) {
            console.log('[v13] Allowance is too low, requesting approval...')
            statusDiv.textContent = \`Please approve \${priceInUsdc} USDC...\`
            
            const approveTx = await usdcContract.approve(CONTRACT_ADDRESS, price)
            console.log('[v13] Approval transaction sent:', approveTx.hash)
            
            statusDiv.textContent = 'Waiting for approval confirmation...'
            await approveTx.wait() // Wait for 1 confirmation
            
            console.log('[v13] Approval confirmed!')
          } else {
            console.log('[v13] Approval already sufficient.')
          }
        } else {
          console.log('[v13] Price is 0. Skipping approval.')
        }
        ...
        console.log('[v13] Preparing takeover transaction...')

        const deadline = Math.floor(Date.now() / 1000) + 300 // 5-minute deadline
        const uri = "" 
        const channelOwner = userAddress // Use the checksummed address
        ...
        console.log('[v13] Takeover transaction sent:', takeoverTx.hash)
        statusDiv.textContent = 'Waiting for confirmation...'
        
        await takeoverTx.wait() 
        
        console.log('[v13] Transaction confirmed!')
        statusDiv.textContent = 'Success! Redirecting...'
        statusDiv.className = 'status success'
        
        setTimeout(() => {
          window.location.href = GAME_SERVICE_URL
        }, 2000)
        
      } catch (error) {
        console.error('[v13] Payment error:', error)
    ...
    console.log('[v13] Click handler attached')
    statusDiv.textContent = 'Ready to play'
  </script>
</body>
</html>`

    console.log("[v13] Payment frame HTML generated")

    res.setHeader("Content-Type", "text/html; charset=utf-8")
    res.setHeader("Cache-Control", "no-cache, no-store, must-revalidate")
    res.status(200).send(html)

    console.log("[v13] Payment frame response sent")
  } catch (e) {
    console.error("[v13] FATAL ERROR in payment frame:", e.message)
