//
// This is the full content for api/payment-frame.js (v12 - Lowercase Address Fix)
//
module.exports = async function handler(req, res) {
  console.log("[v12] /api/payment-frame called - Method:", req.method)
  ...
  try {
    ...
    if (!GAME_URL || !PUBLIC_URL) {
      console.error("[v12] ERROR: Missing GAME_URL or PUBLIC_URL env vars")
      return res.status(500).send("Server configuration error: Missing required environment variables.")
    }

    console.log("[v12] Payment frame loaded")
...
<body>
  ...
  <script type="module">
    console.log('[v12] Payment frame script starting')
    
    const { ethers } = await import('https://esm.sh/ethers@5.7.2')
    
    // --- START CONFIGURATION ---
    // --- NEW CHECKSUM FIX: Use all-lowercase addresses ---
    ...
    payButton.addEventListener('click', async () => {
      console.log('[v12] Button clicked!')
      ...
      try {
        ...
        console.log('[v12] Fetching price from /api/get-price...')
        ...
        console.log(\`[v12] Current price: \${price.toString()} (\${priceInUsdc} USDC)\`)
        console.log(\`[v12] Current epochId: \${epochId}\`)

        if (price.isZero()) {
          console.log('[v12] Price is zero. Switching to free claim.')
        ...
        console.log('[v12] Importing Farcaster SDK')
        const { default: sdk } = await import('https://esm.sh/@farcaster/miniapp-sdk')
        
        console.log('[v12] SDK imported, calling ready()')
        await sdk.actions.ready()
        
        console.log('[v12] Getting Ethereum provider from wallet')
        ...
        console.log('[v12] Provider obtained, requesting accounts')
        ...
        console.log('[v12] User address (raw):', rawUserAddress)
        // We still checksum the user's address, as this is good practice
        const userAddress = ethers.utils.getAddress(rawUserAddress)
        console.log('[v12] User address (checksummed):', userAddress)
        // --- END FIX ---
        ...
        // This will now work because the addresses are lowercase
        ...
        if (price.gt(0)) { // Only run if price is greater than 0
          ...
          console.log('[v12] Price > 0. Checking allowance...')
          
          const currentAllowance = await usdcContract.allowance(userAddress, CONTRACT_ADDRESS)
          console.log(\`[v12] Current allowance: \${currentAllowance.toString()}\`)
          
          if (currentAllowance.lt(price)) {
            console.log('[v12] Allowance is too low, requesting approval...')
            statusDiv.textContent = \`Please approve \${priceInUsdc} USDC...\`
            
            const approveTx = await usdcContract.approve(CONTRACT_ADDRESS, price)
            console.log('[v12] Approval transaction sent:', approveTx.hash)
            
            statusDiv.textContent = 'Waiting for approval confirmation...'
            await approveTx.wait() // Wait for 1 confirmation
            
            console.log('[v12] Approval confirmed!')
          } else {
            console.log('[v12] Approval already sufficient.')
          }
        } else {
          console.log('[v12] Price is 0. Skipping approval.')
        }
        ...
        console.log('[v12] Preparing takeover transaction...')
        ...
        console.log('[v12] Takeover transaction sent:', takeoverTx.hash)
        statusDiv.textContent = 'Waiting for confirmation...'
        
        await takeoverTx.wait() 
        
        console.log('[v12] Transaction confirmed!')
        ...
        console.error('[v12] Payment error:', error)
    ...
    console.log('[v12] Click handler attached')
    statusDiv.textContent = 'Ready to play'
  </script>
</body>
</html>`

    console.log("[v12] Payment frame HTML generated")

    res.setHeader("Content-Type", "text/html; charset=utf-8")
    res.setHeader("Cache-Control", "no-cache, no-store, must-revalidate")
    res.status(200).send(html)

    console.log("[v12] Payment frame response sent")
  } catch (e) {
    console.error("[v12] FATAL ERROR in payment frame:", e.message)
